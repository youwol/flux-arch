...
export namespace RemoteAndersonian {
    ...
    export class Module extends ModuleFlow {
        ...
        constructor(params) {
            ...
            this.addInput("input",  inputDescription, this.createRemote )
            this.remote$ = this.addOutput("remote", {})

            let conf = this.getConfiguration<PersistentData>()
            if(conf.emitInitialValue)
                this.createRemote(undefined,conf, {})
        }

        createRemote(_, config: PersistentData, context: any){
            this.remote$.next({data:new ArcheFacade.AndersonianRemote({...config, ...{stress:true}}), context})
        }
    }

    let inputDescription = {
        description: `Triggerring this input construct an Andersonian remote stress field. No data beside configuration is needed.`,
        mandatory: {}
    } 
}
