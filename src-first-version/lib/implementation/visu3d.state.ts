...
import { DataFrame, decodeDataFrame } from '@youwol/flux-pack-dataframe';
import { ApplyAttributes } from '@youwol/flux-pack-kepler';
...
export class Visu3dState{
    ...
    constructor(public readonly projectName: string, public readonly tree: TreeViewState, public readonly drive: FileSystem.Drive, subscriptions : Array<Subscription> ){
    ...
    buildSimpleMesh( node: ArcheMeshNode ) :  Observable<Mesh>{
        ...
        return this.drive.readAsText(node.fileId).pipe(
            map( meshText =>  {
                let decoded = decodeGocadTS(meshText, { collapse: false })
                return decoded[0];
            }),
