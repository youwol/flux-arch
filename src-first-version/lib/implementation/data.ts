...
export abstract class Environment{
    ...
    readonly drive: FileSystem.Drive
    readonly folder: FileSystem.Folder
...
export function needSolve( commands: Array<Tree.Command<ArcheNode>>){

    let r = commands.reduce( (acc,command) =>{

        if( command instanceof Tree.AddChildCommand 
            && command.childNode instanceof ArcheDiscontinuityMeshNode )
            return true 
        
        if( command instanceof Tree.AddChildCommand && command.childNode instanceof ArcheConstraintNode )
            return true 
            
        if( command instanceof Tree.AddChildCommand && command.childNode instanceof ArcheRemoteNode )
            return true 
        
        if( command instanceof Tree.ReplaceAttributesCommand && command.node instanceof ArcheMaterialNode )
            return true 
 
        if( command instanceof Tree.ReplaceAttributesCommand && command.node instanceof ArcheBoundaryConditionNode )
            return true         
        
        if( command instanceof Tree.ReplaceAttributesCommand && command.node instanceof ArcheConstraintNode )
            return true         
                
        if( command instanceof Tree.ReplaceAttributesCommand && command.node instanceof ArcheRemoteNode )
            return true         
                        
        if( command instanceof Tree.ReplaceNodeCommand && command.newNode instanceof ArcheMaterialNode )
            return true         
...
export class ProjectState{
    ...
    constructor(  
        ...
        public readonly withCommands: Array<Tree.Command<ArcheNode>> = [],  
        ...
        if(this.withCommands.filter(cmd => !(cmd instanceof Tree.InitCommand) ).length==0 && !node)
...
export class TreeViewState extends  Tree.State<ArcheNode> {
            ...
            (file: FileSystem.File) => {
                ...
                if(parentNode instanceof ArcheDiscontinuityNode){
                    ...
                    reader.addEventListener("loadend",  () => {
                        let boundingBox = getBoundingBox( decodeGocadTS(reader.result as string, { collapse: false }))
