...
import { distinctUntilChanged, filter, map, mergeMap, scan, share, switchMap, take, tap, withLatestFrom } from 'rxjs/operators'
import * as _ from 'lodash'
import { decodeGocadTS } from '@youwol/io'
import { extractNewObservationMeshNodes, findChild, findChildren, getBoundingBox } from './utils'
import { ArcheFacade } from '../arche.facades'
import { RootArcheNode, ArcheDiscontinuityNode, ArcheMeshNode, 
...
import { Environment, ProjectState, Solution, needSolve, newProjectNode, TreeViewState} from './data'
...
: Tree.Command<ArcheNode>{
    ...
    if(component instanceof ArcheFacade.Material){
        ...
        return new Tree.ReplaceNodeCommand<ArcheNode>(oldNode, material) 
    }
    if(component instanceof ArcheFacade.AndersonianRemote){
        ...
        return new Tree.AddChildCommand<ArcheNode>(folder, remote) 
    }

}
function applyCommands( 
    tree: TreeViewState, 
    commands: Array<Tree.Command<ArcheNode>>){
    
    let cmds = commands.filter(cmd => !(cmd instanceof Tree.InitCommand))
...
export class StateMgr {
    ...
    withCommands = new Array<Tree.Command<ArcheNode>>()
    withCommandComponents = new Array<Tree.Command<ArcheNode>>()
    nodesWatched = new Array<string>()
    lastSolution : {solution: Solution, commandRef: Tree.Command<ArcheNode> }

    constructor(
        {   id, name, fromState, withCommands, withComponents, environment, context}:
        {   id: string, name: string, fromState:ProjectState, withCommands:Array<Tree.Command<ArcheNode>>, 
            ...
            let needEmit = allCommands.filter( cmd => !(cmd instanceof Tree.InitCommand)).length == 0
            ...
            this.subscriptions.push(
                ...
                this.tree.directUpdates$.pipe( 
                    scan( (acc, update) => {
                        ...
                        return updates.filter( (update,i)=> i<updates.length-1 ? !(update.command instanceof Tree.InitCommand) : true)  
                    }, [] ),
                    tap( all =>{
                        this.withCommands = all.map( update => update.command ) 
                        if(this.withCommands.filter( cmd => !(cmd instanceof Tree.InitCommand)).length>0)
                            console.log("###===> got command "+this.id, this.withCommands)
                    }),
                    map( ( allUpdates: Array<Tree.Updates<ArcheNode>> ) => {
            ...
            this.subscriptions.push(
                ...
                this.tree.directUpdates$.pipe( 
                    filter( (updates: Array<Tree.Updates<ArcheNode>>) => {
                    ...
                    switchMap( ({model, lastUpdate} : {model:ArcheFacade.Model, lastUpdate:Tree.Updates<ArcheNode>}) => {

                        return this.tree.environment
                        .solve(model, (lastUpdate.newTree as RootArcheNode).process$)
                        .pipe( map( (solution) => ({solution, lastUpdate})) )
                    }),
                    tap( ({solution, lastUpdate} : {solution:Solution, lastUpdate:Tree.Updates<ArcheNode>}) => {
            ...
            let plugResolution = ( obs$: Observable<{solution, nodes}>) => {
                ...
                this.subscriptions.push(
                    ...
                    .subscribe(({file, oldMeshNode, solutionId}: 
                        {file: FileSystem.File, oldMeshNode:ArcheObservationMeshNode, solutionId:string}) => {
    ...
    saveMesh( file: FileSystem.File, mesh: Mesh ){
        ...
        this.tree.environment.drive.updateFile(file, new Blob([s])).pipe(
