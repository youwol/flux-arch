...
import { DataFrame, encodeDataFrame } from '@youwol/flux-pack-dataframe'
...
export class WasmWorker{
    ...
    fetchWasm() {
        return fetch(`/api/cdn-backend/libraries/youwol/${NAME}/${VERSION}/assets/arche.js`)
        .then( d => d.text())
    }
    createWorker(){
...
export class LocalEnvironment extends Environment{
    ...
    constructor(
        public readonly drive: FileSystem.Drive, 
        public readonly folder: FileSystem.Folder,
        ...
        from(this.wasmWorker.fetchWasm()).pipe(
            ...
            mergeMap( (archeSrcContent) => {
                return this.drive.readAsText(meshFileId).pipe( map(content=> {
                    return { archeSrcContent, mesh: decodeGocadTS( content, { collapse: false })[0]}
                }))
            })
