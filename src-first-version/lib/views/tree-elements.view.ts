...
export function progressUI(event: FileSystem.EventIO) {

    let p = Math.floor(100 * event.bytesTransfered / event.bytesTotal)
    return {
        class: "w-100 progress-bar progress-bar-striped  bg-success " + (event.step == FileSystem.Step.FINISHED ? "arche-progress finished" : ""),
...
export function headerView(tree: TreeViewState, node: ArcheNode, root: RootArcheNode, subscriptions) {
    ...
    return node.signals$.pipe(
        map((s) => {
            return (s.type != 'start-renaming')
                ? headerWidget
                : Tree.headerViews.headerRenamed(node, tree, node.signals$)
        }))
}
