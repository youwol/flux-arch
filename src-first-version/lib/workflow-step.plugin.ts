...
import { Flux, BuilderView, Schema, PluginFlow, SideEffects, Property, Orchestrator, StaticStorage, uuidv4 } from '@youwol/flux-lib-core'
...
export namespace WorkflowStep {
    ...
    @Flux({
        ...
        compatibility: {
            Component: {
                condition: (mdle) => mdle instanceof ProjectMgr.Module,
                description: "A step should be associated to a project manager module"
            }
        }
    })
    @BuilderView({
    ...
    export class Module extends PluginFlow<ProjectMgr.Module>  implements SideEffects {
        ...
        constructor(params) { 
            ...
            this.uid = this.parentModule.getConfiguration<ProjectMgr.PersistentData>().projectName +"_"+uuidv4()
            let managerId = this.getConfiguration<PersistentData>().managerId;
