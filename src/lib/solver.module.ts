...
import { Flux, BuilderView, ModuleFlow, Pipe, Schema, Property, Report, LogLevel, uuidv4 } from '@youwol/flux-lib-core'
import { ArcheFacade } from './arche.facades';
import { NAME, VERSION } from '../auto-generated';

import { Environment } from './implementation/data';
import { createWorker, processArcheTaskInWorker } from './implementation/local-environment';
...
export namespace Solver {
    ...
    export class PersistentData {
        ...
        constructor({ type, maxIteration, tolerance }: { type?: string, maxIteration?: number, tolerance?: number } = {}) {

            this.type = type != undefined ? type : 'seidel'
            this.maxIteration = maxIteration != undefined ? maxIteration : 200
            this.tolerance = tolerance != undefined ? tolerance : 1e-9
        }
    }
    ...
    export class Module extends ModuleFlow {
        ...
        solution$: Pipe<ArcheFacade.Solution>

        constructor(params) {
            super(params)

            this.addInput("input", inputDescription, this.solve)
            this.solution$ = this.addOutput("solution", {})
        }

        solve(data: {model:ArcheFacade.Model, environment:Environment }, config: PersistentData, context: any) {
            let model : ArcheFacade.Model = undefined
            if( data instanceof ArcheFacade.Model )
                model = data
            else if( data.model && (data.model instanceof ArcheFacade.Model) )
                model = data.model
            this.solveMultiThreaded(model, config, context, this.solution$)
        }


        solveMultiThreaded(inputData: ArcheFacade.Model, configuration: PersistentData, context: any, solution$: Pipe<ArcheFacade.Solution>) {
            ...
            fetch(`/api/cdn-backend/libraries/youwol/${NAME}/${VERSION}/assets/arche.js`).then(
                ...
                let itemsToTransfert = inputData.surfaces
                ...
                let worker = createWorker(processArcheTaskInWorker)
    ...
    let inputDescription = {
        description: `Triggerring this input trigger the solver. No data beside configuration is needed.`,
        mandatory: {
            description: 'provide an Arche model',
            oneOf:[
                {description: 'provides a model directly',
                 test: (input) => input instanceof ArcheFacade.Model},
                {description: 'provides a model in data.model attribute',
                test: (input) => input.model && (input.model instanceof ArcheFacade.Model)}
            ]
