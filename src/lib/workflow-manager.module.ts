...
import { Property, Flux, BuilderView, Orchestrator, SideEffects, Pipe, RenderView} from '@youwol/flux-core'
...
export namespace WorkflowManager {
    ...
    class WfNode extends ImmutableTree.Node{
    ...
    export class Module extends Orchestrator implements SideEffects {
        ...
        tree$ : Observable<ImmutableTree.State<WfNode>>
        treeview : ImmutableTree.View<WfNode>
        ...
        constructor(params){ 
            ...
            this.tree$ = this.newLinks$.pipe(
                ...
                map( edges => {
                    ...
                    return new ImmutableTree.State<WfNode>({rootNode:root})
                })
            )
        ...
        registerStep(plugin: WorkflowStep.Module){
            ...
            this.names[plugin.uid] = plugin.parentModule.getPersistentData<ProjectMgr.PersistentData>().projectName
    ...
    function renderHtmlElement( mdle : Module ){
        ...
        let headerView =  (state: ImmutableTree.State<WfNode>, node:WfNode) =>{
        ...
        /*
        mdle.treeview = new ImmutableTree.View<WfNode>( mdle.tree$ , {actions, headerView}, mdle.subscriptions)
